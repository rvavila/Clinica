...
export default {
  async fetch(request, env) {
    ...
    try {
      ...
      if(path==='/api/notifications'&&request.method==='GET')return json((await env.DB.prepare("SELECT * FROM notifications WHERE recipient_user_id=? AND message LIKE '%cancelada%' ORDER BY created_at DESC,id DESC LIMIT 30").bind(user.id).all()).results,200,origin);
